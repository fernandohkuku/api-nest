...
export class IdeaService {
    ...
    async showAll():Promise<IdeaRO[]> {
        const ideas = await this._ideaRepository.find({relations:["author",'upvotes','downvotes','comments']});
    ...
    async read(id: string):Promise<IdeaRO>{
        const  idea = await this._ideaRepository.findOne({ where: { id }, relations:["author", "upvotes", "downvotes",'comments'] });
        if(!idea){
    ...
    async update(id: string, userId:string, data:Partial<IdeaDTO>):Promise<IdeaRO> {
        ...
        idea = await this._ideaRepository.findOne({where:{ id }, relations:["author", "comments"]})
        return this.toResponseObject(idea)
    }

    async destroy(id: string,userId:string) {
        const  idea = await this._ideaRepository.findOne({ where: { id }, relations:["author", "comments"] });
        if(!idea){
            throw new HttpException("Not Found", HttpStatus.NOT_FOUND)
        }
        this.ensureOwnership(idea, userId)
    ...
    async upvote(id:string, userId:string){
        let idea = await this._ideaRepository.findOne({where : {id}, relations:["author", "upvotes", "downvotes", "comments"]})
    ...
    async bookmark(id:string, userId:string){
        ...
        const user = await this._userRepository.findOne({where:{id:userId},relations:["bookmarks"]});
