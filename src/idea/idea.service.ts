...
export class IdeaService {
    ...
    async showAll():Promise<IdeaRO[]> {
        const ideas = await this._ideaRepository.find({relations:["author",'upvotes','downvotes']});
    ...
    async read(id: string):Promise<IdeaRO>{
        const  idea = await this._ideaRepository.findOne({ where: { id }, relations:["author", "upvotes", "downvotes"] });
        if(!idea){
    ...
    async upvote(id:string, userId:string){
        let idea = await this._ideaRepository.findOne({where : {id}, relations:["author", "upvotes", "downvotes"]})
